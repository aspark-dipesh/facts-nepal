    ...
    return (
        <div className={classNames?.container}>
            <div className='container mx-auto py-20'>
                <Headings title={title} className={classNames?.title} path={hasViewAll ? "/news-and-blogs" : undefined} />
                <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-10 lg:gap-40 mt-10 place-items-center'>
                    {
                        Publications.map((publication, index) => (
                            <Card isHoverable isPressable isFooterBlurred={footerBlur} key={index} className={cn(
                                "w-full h-full relative p-6",
                                classNames?.card
                            )}>
                                <h2 className="text-black text-2xl font-bold my-3">{publication.title}</h2>
